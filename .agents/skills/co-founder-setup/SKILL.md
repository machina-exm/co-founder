...
```
                    the co-founder-setup skill (run it before anything else)
                           |
...
Read `references/CONVENTIONS-core.md`, then look at the current folder before
...
Create the vault's `scripts/` directory. Copy `references/graph-audit` into
...
2. Name the ONE next command: the vision skill — run it now to turn the stub into the real thing.
