...
Every skill reads this file from `references/CONVENTIONS-core.md` before it reads
...
- **Fresh:** no charter and none of `index.md`, `log.md`, or `wiki/` exists. Setup owns all
  scaffold creation. Every other collection skill stops and runs the co-founder-setup skill before anything else without writing.
- **Partial:** some signature files exist but the Ready signature does not. Preserve every
  existing file, name what is missing, and stop and run the co-founder-setup skill before anything else in re-sync mode.
