...
Before any step, read `references/CONVENTIONS-core.md`. This resolves from the
...
Run the shared contract's Universal preflight. A Fresh, Partial, or Wrong-folder result stops and runs the co-founder-setup skill before anything else (re-sync for Partial), without researching or filing. On Ready,
