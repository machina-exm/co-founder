...
Read `references/CONVENTIONS-core.md` and run its Universal preflight. A Fresh,
Partial, or Wrong-folder result stops and runs the co-founder-setup skill before anything else (re-sync for Partial), without writing. On Ready, read the charter and its path map before the vault.
