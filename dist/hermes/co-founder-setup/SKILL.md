...
description: "One-time setup for the /co-founder collection."
---
...
Read `references/CONVENTIONS-core.md` (in this skill's own folder — from the terminal it is `${HERMES_SKILL_DIR}/references/CONVENTIONS-core.md`), then look at the current folder before
...
- `AGENTS.md` already present: extend it, not overwrite. A lone `CLAUDE.md` is preserved while `AGENTS.md` becomes the single source of truth.
- Neither: create `AGENTS.md` as the single source of truth for all harnesses. Also write `CLAUDE.md` for Claude Code compatibility: link it to `AGENTS.md`, or on Windows, where links need admin rights, create a two-line file containing `@AGENTS.md` instead.
...
Fill the template with the founder's own words from step 2. Target under 150 lines. Then also write the Claude Code compatibility shim per step 1's finding.

Done when: `AGENTS.md` exists as the single source of truth, its `CLAUDE.md` compatibility shim resolves to it, and every section carries their content, with zero unfilled placeholders.
...
The seven hub notes use the fixed filenames from the reference. Also read `references/CONVENTIONS-inbox-ledger.md`, then seed `queue.md`,
...
Create the vault's `scripts/` directory. Copy `references/graph-audit` (from the terminal: `${HERMES_SKILL_DIR}/references/graph-audit`) into
it byte-for-byte as the vault's `graph-audit` script and preserve executable mode (or set it where the
...
1. Run the vault's `graph-audit` script across the complete fresh or migrated vault.
...
2. Name the ONE next command: run `/vision` to turn the stub into the real thing.
